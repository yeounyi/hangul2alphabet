...
def syl_to_alphabet(syl, explain=False):
	...
	# 한글 음소인 경우
    elif re.findall(r'[ㄱ-ㅎ]', syl) != []:
        result = phon_to_alphabet(syl)
        explanation = ['일단 보류. 추후 수정'] * len(result)
        return pd.DataFrame(zip(result, explanation), columns=[syl, 'explanation'])

    # 한글이 아닌 경우 
    else:
    	return pd.DataFrame(zip(syl, '한글이 아니라서 그대로 반환'), columns=[syl, 'explanation'])
...
st.write('\* 띄어쓰기는 \'_\'로, 그 외의 한글이 아닌 문자는 그대로 반환됩니다')

user_input = st.text_input("변환하고 싶은 '한글'을 입력하세요: ")
# 띄어쓰기는 _ 로 대체 
...
link = '[GitHub](https://github.com/yeounyi/hangul2alphabet)'
